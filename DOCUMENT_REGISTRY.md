# DOCUMENT REGISTRY

> **Official Register of Madina Barakasi Documentation**

---

## Document Information

| Field          | Value              |
| -------------- | ------------------ |
| Document ID    | MB-REG-001         |
| Version        | 1.1.9              |
| Status         | Approved           |
| Owner          | Documentation Team |
| Classification | Canonical          |
| Language       | English            |
| Created        | 2026-08-07         |
| Last Updated   | 2026-09-01         |

---

# Purpose

This document is the official registry of all documentation maintained within the Madina Barakasi Documentation Library.

Every official document must be registered here.

The registry reflects documents that actually exist within the current Documentation Library.

---

# Status Legend

| Status     | Description                  |
| ---------- | ---------------------------- |
| Draft      | Under development            |
| Review     | Under review                 |
| Approved   | Official document             |
| Released   | Published                    |
| Planned    | Identified but not yet created|
| Archived   | Historical version            |

---

# Classification Legend

| Type          | Description                  |
| ------------- | ---------------------------- |
| Canonical     | Primary source of truth      |
| Standard      | Engineering standard         |
| Policy        | Governance policy            |
| Guideline     | Recommended practice         |
| Specification | Technical specification      |
| Registry      | Project registry             |
| Template      | Documentation template       |
| SOP           | Standard Operating Procedure |
| ADR           | Architecture Decision Record |

---

# Core Registries

| ID              | Document          | Version | Status   | Classification |
| --------------- | ----------------- | ------- | -------- | -------------- |
| MB-INDEX-000    | Master Index      | 1.0.0   | Approved | Canonical      |
| MB-REG-001              | Document Registry                                  | 1.1.9   | Approved | Canonical      |
| MB-ADR-REG-001  | ADR Registry      | 1.0.0   | Approved | Registry       |
| MB-PORT-REG-001 | Ecosystem & Project Portfolio Registry | 0.1.7 | Draft | Registry |
| MB-PLATFORM-CRM-CHK-001 | Madina Platform / CRM Canonical Project Checkpoint | 0.1.0 | Draft | Registry |
| MB-SABONO-RETAIL-CHK-001 | SABONO Retail Canonical Project Checkpoint | 0.1.3 | Draft | Registry |

---

# Volume I — Foundation

| ID     | Document    | Version | Status | Classification |
| ------ | ----------- | ------- | ------ | -------------- |
| MB-004 | Master Plan | 1.0.1   | Draft  | Canonical      |

> MB-000 through MB-003 are referenced by historical registry content but are not present as files in the current repository and are therefore not registered as current documents.

---

# Volume II — Architecture

| ID     | Document              | Version | Status | Classification |
| ------ | --------------------- | ------- | ------ | -------------- |
| MB-100 | System Architecture   | 0.1.1   | Draft  | Canonical      |
| MB-101 | Data Architecture     | 0.1.1   | Draft  | Canonical      |
| MB-102 | Engineering Standards | 0.1.0   | Draft  | Standard       |
| MB-ADR-002 | Application-Domain Core Package Boundary | 0.1.0 | Draft | ADR |
| MB-ADR-003 | LocalStorage Transactional Aggregate Snapshot | 0.1.0 | Draft | ADR |

---

# Volume III — Design & Experience

| ID      | Document      | Version | Status | Classification |
| ------- | ------------- | ------- | ------ | -------------- |
| MB-200              | Design System                  | 0.1.2   | Draft    | Specification   |
| MB-201              | Visual Identity Foundation     | 0.1.2   | Draft    | Specification   |
| MB-STD-007 | Symbol Design Principles | 0.1.0 | Draft | Standard |
| MB-ADR-001 | Symbol Independent From Name | 0.1.0 | Draft | ADR |
| MB-DEC-001 | Brandmark Architecture | 1.0.0 | Approved | Design Decision |
| MB-DEC-002 | Arabic Typography Architecture | 1.0.0 | Approved | Design Decision |
| MB-DEC-003          | Latin Wordmark and Horizontal Lockup Architecture | 1.0.0 | Approved | Design Decision |
| MB-DEC-INDEX        | Design Decisions Index         | 0.1.3   | Draft    | Registry        |
| MB-DESIGN-STRUCTURE | Design System Structure | 0.1.0 | Draft | Guideline |

---

# Volume IV — Engineering

| ID          | Document               | Version | Status | Classification |
| ----------- | ---------------------- | ------- | ------ | -------------- |
| MB-ENG-001  | Evidence-Based Engineering Execution Protocol | 0.1.0   | Draft  | Standard       |
| MB-DATA-001 | Master Data Dictionary | 0.1.0   | Draft  | Canonical      |
| MB-DATA-002 | ER Diagram             | 0.1.0   | Draft  | Specification  |
| MB-API-001  | API Standards          | 0.1.0   | Draft  | Standard       |

> MB-DATA-001, MB-DATA-002, and MB-API-001 are registered Draft entries but are not currently present as files in the repository.

---

# Volume V — Business Modules

| ID         | Document   | Version | Status  | Classification |
| ---------- | ---------- | ------- | ------- | -------------- |
| MB-CRM-001 | CRM Module | 0.1.0   | Draft   | Specification  |
| MB-CRM-002 | Business Domains | 0.1.0 | Draft | Specification |
| MB-CRM-003 | Business Entities | 0.1.0 | Draft | Specification |
| MB-CRM-004 | Business Processes | 0.1.0 | Draft | Specification |
| MB-CRM-005 | Business Rules | 0.1.0 | Draft | Specification |
| MB-CRM-006 | Data Contracts | 0.1.0 | Draft | Specification |
| MB-CRM-007 | Application Implementation | 0.1.0 | Draft | Specification |
| MB-CRM-008 | Validation | 0.1.0 | Draft | Specification |

---

# Volume VI — Operations

| ID         | Document               | Version | Status  | Classification |
| ---------- | ---------------------- | ------- | ------- | -------------- |
| MB-SOP-001 | Documentation Workflow | 0.1.0   | Draft   | SOP            |

---

# Volume VII — Knowledge Base

| ID        | Document             | Version | Status  | Classification |
| --------- | -------------------- | ------- | ------- | -------------- |
| MB-KB-001 | Engineering Handbook | 0.1.0   | Planned | Guideline      |

---

# Statistics

The statistics below describe the current registry entries.

| Metric            | Value |
| ----------------- | ----: |
| Current Documents |    35 |
| Approved          |     6 |
| Draft             |    28 |
| Planned           |     1 |

---

# Change Policy

Every official document:

- must have a unique Document ID;
- must belong to exactly one Volume or Core Registry;
- must have a version;
- must have a status;
- must be listed in this registry;
- must correspond to an actual repository artifact when marked as current.

Planned entries may identify future documents but must not be represented as existing repository artifacts.

---

**Madina Barakasi Documentation Library**
